...
function &init_users()
{
    ...
    $cls = new App\Services\Passport\Passport($GLOBALS['_CFG']['integrate_code'], $cfg);
...
function clear_tpl_files($is_cache = true, $ext = '')
{
    ...
    if ($is_cache) {
        $cache_dir = storage_path($tmp_dir . '/caches/');
        $dirs[] = storage_path($tmp_dir . '/query_caches/');
        $dirs[] = storage_path($tmp_dir . '/static_caches/');
        for ($i = 0; $i < 16; $i++) {
            $hash_dir = $cache_dir . dechex($i);
            $dirs[] = $hash_dir . '/';
        }
    } else {
        $dirs[] = storage_path($tmp_dir . '/compiled/');
        $dirs[] = storage_path($tmp_dir . '/compiled/admin/');
    }
